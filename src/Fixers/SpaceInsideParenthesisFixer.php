...
final class SpaceInsideParenthesisFixer extends AbstractFixer
{
    ...
    protected function fixParenthesisInnerEdge(
        ...
        if (
            ! $tokens[$end - 1]->isWhitespace($this->singleLineWhitespaceOptions) &&
            ! str_contains($tokens[$end - 1]->getContent(), "\n")
        ) {
        ...
        if (
            ! $tokens[$start + 1]->isWhitespace($this->singleLineWhitespaceOptions) &&
            ! str_contains($tokens[$start + 1]->getContent(), "\n")
        ) {
